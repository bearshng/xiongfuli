...
2. RubyDevKit - You can download RubyDevKit [here](http://rubyinstaller.org/downloads/ "here").The version of RubyDevKit should be in accordance with that of Ruby.Download the file, unzip it in a directory of your liking and go to the directory using cmd.Then,Run following commands.`$ruby dk.rb init`，you will get config.yml file.In the end ,Use `$ruby setup.rb` to install RubyDevKit .If there is no setup.rb ,`$ruby dk.rb install` works as well.
3. Rubygems
